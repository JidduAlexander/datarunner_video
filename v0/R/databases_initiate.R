...
db_user <- tibble(
  user_id     = c(1, 2),
  user_name   = c("Admin", "Jiddu"),
  email       = c("admin@datarunner.tech", "san@nesware.net"),
  pw_hash     = map_chr(c("admin", "runner"), function(x) paste(as.character(hash(charToRaw(x))), collapse = "")),
  date_joined = date()
)
